...
class M_galeri extends CI_Model{
	...
	function simpan_galeri($judul,$album,$user_id,$user_nama,$gambar,$client,$url,$deskripsi,$slug){
		$this->db->trans_start();
            $this->db->query("insert into tbl_galeri(galeri_judul,galeri_album_id,galeri_pengguna_id,galeri_author,galeri_gambar,galeri_nama_client,galeri_url_web,galeri_deskripsi,galeri_slug) values ('$judul','$album','$user_id','$user_nama','$gambar','$client','$url','$deskripsi','$slug')");
	...
	function get_galeri_by_kode($kode){
		$hsl=$this->db->query("SELECT tbl_galeri.*,DATE_FORMAT(galeri_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_galeri where galeri_id='$kode'");
		return $hsl;
	}

	function limit_galeri(){
		$hsl=$this->db->query("SELECT * FROM tbl_galeri ORDER BY galeri_tanggal DESC limit 2");
		return $hsl;
	}
}
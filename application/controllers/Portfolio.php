...
class Portfolio extends CI_Controller{
	...
	function index(){
        ...
        $config['base_url'] = base_url() . 'galeri/index/';
			...
			$x['limit']=$this->m_galeri->get_galeri_home();
			$this->load->view('depan/portfolio',$x);
	}
	
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_galeri', array('galeri_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['galeri_id'];
			// $this->db->query("UPDATE tbl_tulisan SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_galeri->get_galeri_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['galeri_id'];
			$x['judul']=$row['galeri_judul'];
			$x['image']=$row['galeri_gambar'];
			$x['portfolio'] =$row['galeri_deskripsi'];
			$x['tanggal']=$row['galeri_tanggal'];
			$x['author']=$row['galeri_author'];
			// $x['link_driv']=$row['link_driv'];
			// $x['download']=$row['link_driv'];
			$x['album']=$row['galeri_album_id'];
			$x['client']=$row['galeri_nama_client'];
			$x['url']=$row['galeri_url_web'];
			$x['slug']=$row['galeri_slug'];
			...
			$this->load->view('depan/portfolio_detail',$x);
		}else{
			redirect('portfolio');
		}
	}
